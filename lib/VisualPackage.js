...
class VisualPackage {
     ...
     */
    static createVisualPackage(rootPath, force) {
        return new Promise((resolve, reject) => {
            VisualGenerator.generate(rootPath, force).then(() => {
                VisualPackage.loadVisualPackage(rootPath).then(resolve).catch(reject);
            }).catch(reject);
        });
    }
