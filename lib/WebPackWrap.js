...
class WebPackGenerator {
    ...
    async getEnvironmentDetails() {
        ...
        let env = {};
        env.operatingSystem = await os.platform();
        env.nodeVersion = await (await exec('node -v')).stdout.match(regexFullVersion)[0];
        let toolsVersion;
        try {
            toolsVersion = (await exec('npm list powerbi-visuals-tools version')).stdout.match(regexFullVersion)[0];
        } catch (error) {
            ConsoleWriter.warn(`"powerbi-visuals-tools" are not installed locally: ${error}`);
            try {
                toolsVersion = (await exec('npm list -g powerbi-visuals-tools version')).stdout.match(regexFullVersion)[0];
            } catch (err) {
                ConsoleWriter.warn(`"powerbi-visuals-tools" are not installed: ${err}`);
            }
        }
        env.toolsVersion = toolsVersion;
    ...
    async configureCustomVisualsWebpackPlugin(visualPackage, options, tsconfig) {
        ...
        let environment = await this.getEnvironmentDetails();
        pluginConfiguration.env = environment;
