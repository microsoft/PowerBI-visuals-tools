...
class WebPackGenerator {
    ...
    async getEnvironmentDetails() {
        ...
        let requiredNode = '12.17.0';
        let env = {};
        env.nodeVersion = (await exec('node -v')).stdout.match(regexFullVersion)[0];
        env.operatingSystem = await os.platform();
        env.osVersion = (compareVersions.compare(env.nodeVersion, requiredNode, '<')) ? `os.version() method require NodeJS version ${requiredNode} or above` :  await os.version();
